...
class App(customtkinter.CTk):
    ...
    def _create_hidden_legacy_controllers(self):
        ...
        self.accounts_control = AccountsControl(self.legacy_host, self.update_label, self, render_ui=False)
        self.control_frame = ControlFrame(self.legacy_host, render_ui=False)
        self.main_menu = MainMenu(self.legacy_host, render_ui=False)
        self.config_tab = ConfigTab(self.legacy_host, render_ui=False)
        ...
        if button is None:
            for text, candidate in self.lobby_buttons.items():
                if text.strip().lower() == "make lobbies & search game":
                    ...
                    break
        if button is None:
            self.log_manager.add_log("❌ UI button 'Make lobbies & search game' not found in unified_app.py")
            return False

        def invoke_button():
            try:
                button.invoke()
                self.log_manager.add_log("✅ AUTO: invoke() on unified_app.py button 'Make lobbies & search game'")
            except Exception as error:
                self.log_manager.add_log(f"❌ Failed to invoke unified_app.py button: {error}")
