...
class AccountsControl(customtkinter.CTkTabview):
    ...
    def start_booster_selected(self):
        ...
        for i, acc in enumerate(selected_accounts):
            # Задержка 6 секунд МЕЖДУ запусками (кроме первого)
            if i > 0:
            ...
            try:
                ...
                time.sleep(1)  # Короткая пауза для проверки запуска
                ...
                self._logManager.add_log(
                    f"✅ [{acc.login}] Activity booster запущен ({min_minutes}-{max_minutes} мин., "
                    f"games: {games_info}, log: {log_path})"
                )
            except FileNotFoundError:
                self._logManager.add_log("❌ Не найден Node.js (команда node)")
                break  # Прерываем цикл при ошибке Node.js
            except Exception as exc:
                self._close_booster_log(acc.login)
                self._logManager.add_log(f"❌ [{acc.login}] Ошибка запуска booster: {exc}")

        self.accountsManager.selected_accounts.clear()
        self._logManager.add_log("🎮 Все бустеры запущены по очереди!")
        self.update_label()
        self._refresh_modern_levels_ui()  # Обновляем UI уровней
